...
import { OpenVaultManager } from './openvaultManager';
import { QRCodePanel } from './qrCodePanel';
import { PasswordGeneratorPanel } from './passwordGeneratorPanel';
import { OpenVaultTreeProvider } from './treeProvider';

export function activate(context: vscode.ExtensionContext) {
    const manager = new OpenVaultManager();
    const treeProvider = new OpenVaultTreeProvider();
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('openvault.seal', async (uri: vscode.Uri) => {
            try {
                    ...
                    if (result.success) {
                        ...
                        if (result.password) {
                            ...
                            if (action === 'Copy Password') {
                                ...
                                QRCodePanel.createOrShow(context.extensionUri, result.password);
                            }
                        } else {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('openvault.unlock', async (uri: vscode.Uri) => {
            try {
                const filePath = uri?.fsPath || vscode.window.activeTextEditor?.document.fileName;
                if (!filePath) {
                    vscode.window.showErrorMessage('No file selected');
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('openvault.generatePassword', async () => {
            PasswordGeneratorPanel.createOrShow(context.extensionUri);
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('openvault.showQRCode', async () => {
            const password = await vscode.window.showInputBox({
                prompt: 'Enter password to encode as QR Code',
                password: true
            });
            if (password) {
                QRCodePanel.createOrShow(context.extensionUri, password);
            }
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('openvault.listShards', async () => {
            try {
                ...
                const items = shards.map((shard: any) => ({
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('openvault.init', async () => {
            try {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('openvault.keyInfo', async () => {
            try {
